...
var getManager = controller(function(req, res) {
    ...
    .then(function(nodes) {
        ...
        if(identifier !== reservedId) {
            return wsman.isDellSystem(nodes[0].id)
            .then(function(result){
                //var node = result.node;
                if(result.isDell){
                    options.isDell = true; 
                    return nodeApi.getNodeCatalogSourceById(nodes[0].id, 'hardware')
    ...
    .then(function(networkProtocol) {
        ...
        if(options.isDell){
            return redfish.render('wsman.1.0.0.manager.1.0.0.json',
...
var getManagerEthernetInterface = controller(function(req, res) {
    ...
    .then(function(node) {
        return wsman.isDellSystem(node.id)
    })
    .then(function(result) {
        var node = result.node;
        assert.ok(_.isArray(node.obms), 'invalid obmSettings');
        ...
        if(result.isDell){
            options.isDell = true;
            return nodeApi.getNodeCatalogSourceById(node.id, 'hardware');
        } else {
